...
这个项目提供的点云数据均如下**左图**所示：在 $z=0$ 的位置处会广泛地遍布着噪声点云，而目标点云仅仅是高于 $z=0$ 平面的一小部分，为了排除这些地面回波的干扰，使用```pcfitplane```函数（MATLAB的一个内置的基于MSAC算法的平面拟合函数）提取出平面（如下**中图**），然后扣除该平面，得到目标点云（如下**右图**）。

![alt text](figure/f20.png)

### 3.2 Task1

在这个项目中，Task1被用于校对测量程序和以及校准点云的尺度（即将点云数据的尺度映射到真实尺度）。完成这一任务的步骤如下所示：